...
from decoder_only_data_collator import MyDataCollatorWithPadding
...
def get_dataset(args, tokenizer: AutoTokenizer):
    ...
    train_data = DeOnlyDataset(data_path=args.apps_train_files, task_type=args.task_type, max_tokens=args.max_tokens, tokenizer=tokenizer, language=args.language,
                               replace_fields_rule=train_replace_func, data_include_new_fields=train_new_fields_data)
    eval_data = DeOnlyDataset(data_path=args.apps_eval_files, task_type=args.task_type, max_tokens=args.max_tokens, tokenizer=tokenizer, language=args.language,
...
def run_training(args, train_data: DeOnlyDataset, eval_data: DeOnlyDataset, tokenizer: AutoTokenizer, model: CodeGenForCausalLM):
    start_iteration = 0
    if args.resume:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-tokens', default=768, type=int)
